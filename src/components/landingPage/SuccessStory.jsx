...
const testimonials = [
  ...
  {
    id: 2,
    quote: 'Seamless and simple our sign-ups grew with no extra work on our end.',
    handle: '@Devtoolkit',
    bgColor: 'rgb(183, 255, 40)',
    icon: (
      <svg
        width="39"
        height="44"
  ...
  {
    id: 3,
    quote: 'Users loved our cashback perk, and Flowva made the process seamless.',
    handle: '@Teamtoolkit',
    ...
    icon: (
      <svg
        ...
        xmlns="http://www.w3.org/2000/svg"
      >
        <circle cx="22.6001" cy="22.25" r="20.5" stroke="#535862" strokeWidth="3" />
        <path
...
const TestimonialCard = ({ testimonial }) => {
  return (
    ...
    >
      <p className="text-[24px] font-semibold mb-4 font-manrope">{testimonial.quote}</p>
      <div className="flex gap-2 items-center">
        <div className="h-10 w-10 justify-center items-center flex rounded-full bg-white">
          {testimonial.icon}
        </div>
        <h4 className="font-semibold text-[20px]">{testimonial.handle}</h4>
      </div>
    </div>
  );
};
    ...
    <section className="mb-16 px-[14px]">
      ...
      <div className="flex justify-center">
        <div className="w-full md:max-w-[80%]">
          ...
          <div className="hidden md:grid md:grid-cols-3 gap-8 items-start justify-start">
              ...
              {testimonials.map((_, index) => (
                <button
                  ...
                  className={`h-2 w-2 rounded-full transition-all duration-300 ${
                    index === activeIndex ? 'bg-black w-8' : 'bg-gray-400'
                  }`}
